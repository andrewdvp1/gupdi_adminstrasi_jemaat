...
function onSearch() {
    ...
    debounceTimer = setTimeout(() => {
        router.get(route('penyerahan.index'), {
...
function filterTahun() {
    router.get(route('penyerahan.index'), { tahun: tahunFilter.value, search: searchKeyword.value }, { preserveState: true });
}

const exportExcelUrl = computed(() => `/export/penyerahan/excel?tahun=${tahunFilter.value}`);
const exportPdfUrl = computed(() => `/export/penyerahan/pdf?tahun=${tahunFilter.value}`);

function hapus(id) {
    if (confirm('Yakin ingin menghapus data ini?')) {
        router.delete(route('penyerahan.destroy', id));
    }
}
</script>
...
<template>
    <AuthenticatedLayout>
        <Head title="Data Penyerahan Anak" />
        <template #header>
            <h1 class="text-xl font-semibold text-gray-800">Data Penyerahan Anak</h1>
        </template>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 p-4 border-b border-gray-100">
                <div class="flex items-center gap-2">
                    ...
                    <input
                        ...
                        @input="onSearch"
                        placeholder="Cari nama anak, nomor penyerahan..."
                        class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm w-64 focus:ring-blue-500 focus:border-blue-500"
                    />
                </div>
                <Link :href="route('penyerahan.create')" class="bg-blue-700 hover:bg-blue-800 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">
                    + Tambah Data
                </Link>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 text-gray-600 uppercase text-xs">
                        <tr>
                            <th class="px-4 py-3 text-left">No. Penyerahan</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr v-for="item in data.data" :key="item.id" class="hover:bg-gray-50">
                            <td class="px-4 py-3">{{ item.nomor_penyerahan }}</td>
                            <td class="px-4 py-3 font-medium">{{ item.nama_anak }}</td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <Link :href="route('penyerahan.edit', item.id)" class="text-blue-600 hover:text-blue-800 text-xs font-medium">Edit</Link>
